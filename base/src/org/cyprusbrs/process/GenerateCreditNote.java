...
public class GenerateCreditNote extends SvrProcess {
	...
	protected String doIt() throws Exception {
		log.info("M_RMA_ID=" + p_M_RMA_ID 
			...
			invoice.setRMA(mrma); 
		    log.info("Credit Note DocType "+docType.getC_DocTypeInvoice_ID());
			...
			if (!invoice.save())
				throw new IllegalArgumentException("Cannot save Credit Note");
			...
			{	
				...
				for (MRMALine rmaLine : rmaLines)
				{
		            ...
		            if (!invLine.save())
		            {
		                throw new IllegalStateException("Could not create Credit Note line");
		            }
				}
			}
			
		return "CreditNote "+invoice.getDocumentNo();
	}
