...
import org.cyprus.util.UtilTransaction;
...
public class MInOut extends X_M_InOut implements DocAction
{
		...
		for(MInOutConfirm confirm : getConfirmations(true))
		{
			if (!confirm.isProcessed())
			{
				if (!confirm.processIt(MInOutConfirm.DOCACTION_Void))
					throw new CyprusException(confirm.getProcessMsg());
				confirm.saveEx();
			}
		}
	}
	...
	public String prepareIt()
	{
		...
		// Code updated by Mukesh @20211017 AS per discussion "MR should not be complete without PO" @New task in meeting 12-10-2021
		
				/// For MR and Shipment window
//				if((MDocType.DOCBASETYPE_MaterialReceipt.equalsIgnoreCase(dt.getDocBaseType())
//						|| MDocType.DOCBASETYPE_MaterialDelivery.equalsIgnoreCase(dt.getDocBaseType())) && getC_Order_ID()<=0)
		if(((MDocType.DOCBASETYPE_MaterialReceipt.equalsIgnoreCase(dt.getDocBaseType()) && dt.isReturnTrx() == false)
				|| (MDocType.DOCBASETYPE_MaterialDelivery.equalsIgnoreCase(dt.getDocBaseType()) && dt.isReturnTrx() == false)) && getC_Order_ID()<=0)
				{	
					m_processMsg =Msg.getMsg(getCtx(), "Cyprus_MaterialReceipt_NotCompletedWithoutOrder");
					return DocAction.STATUS_Invalid;
				}
		
		if(((MDocType.DOCBASETYPE_MaterialReceipt.equalsIgnoreCase(dt.getDocBaseType()) && dt.isReturnTrx() == true)
				|| (MDocType.DOCBASETYPE_MaterialDelivery.equalsIgnoreCase(dt.getDocBaseType()) && dt.isReturnTrx() == true)) && getM_RMA_ID()<=0)
				{	
					m_processMsg =Msg.getMsg(getCtx(), "Cyprus_Return_NotCompletedWithoutRMA");
					return DocAction.STATUS_Invalid;
				}
	...
	public String completeIt()
	{
		...
		/// Code commented by Mukesh as per discussion by Surya @20220411
			// Code updated by ANshul @20210705
//		MInOutLine[] InOutlines = getLines(false);
//		for (MInOutLine line : InOutlines)
//		{
//			if(UtilTransaction.isCompleteBackDatedRecordTransaction(getMovementDate(),UtilTransaction.MATERIALRECEIPT,getAD_Client_ID(),getAD_Org_ID(),line.getM_Product_ID()))
//			{
//				 m_processMsg =Msg.getMsg(getCtx(), "Cyprus_BankStatement_NotCompletedInBackDate");
//				return DocAction.STATUS_Invalid;
//			}
//		} // ENd of code
